...
accuracy <- mean(fitted.results == test$Survived)
print(accuracy)
...
#Running the predictive model on actual test dataset
...
testoriginal <- testoriginal[!is.na(testoriginal$Fare),]
rownames(testoriginal) <- NULL

test.results <- predict(model,newdata=subset(testoriginal,select=c(2,4,5,6,7,9,11)),type='response')
test.results <- ifelse(test.results > 0.5,1,0)
final.result <- cbind(PassengerID=testoriginal$PassengerId,Survival=test.results)

write.csv(final.result, "solution.csv", row.names=F)
